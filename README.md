...
- **Required IQ layer:** **Foundry IQ** — agentic, permission-aware, *cited* retrieval
  to reduce hallucination (made load-bearing, not decorative)
...
half-remembered past incidents. This agent does that triage in seconds and shows its
work, so a human can approve the fix with confidence.
...
4. **Verify** — a **verifier sub-agent** (LLM-as-judge pattern) tests each hypothesis
   ...
   This is the anti-hallucination core and the project's "Best Use of IQ" story.
...
- **Foundry IQ** — grounded retrieval over `knowledge/` with citations.
...
- **Creativity & originality (15%)** — agentic data-ops is an under-served space.
- **UX & presentation (15%)** — clean cited output + a tight demo.
